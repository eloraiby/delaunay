...
public class Delaunay {
	...
	final int ON_RIGHT = 1;
	final int ON_SEG = 0;
	final int ON_LEFT = -1;

	final int OUTSIDE = -1;
	final int ON_CIRCLE = 0;
	final int INSIDE = 1;

	final double REAL_ZERO = 0.0;
	final double REAL_ONE = 1.0;
	final double REAL_TWO = 2.0;
	final double REAL_FOUR = 4.0;
	...
	private class Face {
		Halfedge he;		/* a pointing half edge */
		int num_verts;		/* number of vertices on this face */
	}

	private class Halfedge {
		Point2d vertex;	/* vertex */
		Halfedge pair;		/* pair */
		Halfedge next;		/* next */
		Halfedge prev;		/* next^-1 */
		Face face;		/* halfedge face */
	}

	private class DelaunaySegment {
		Halfedge rightmost_he;	/* right most halfedge */
		Halfedge leftmost_he;	/* left most halfedge */
		Point2d points[];		/* pointer to points */
		Face faces[];			/* faces of delaunay */
	...
	private class working_set_s {
		Halfedge edges;	/* all the edges (allocated in one shot) */
		Face faces;		/* all the faces (allocated in one shot) */

		int max_edge;		/* maximum edge count: 2 * 3 * n where n is point count */
		int max_face;		/* maximum face count: 2 * n where n is point count */

		int num_edges;		/* number of allocated edges */
		int num_faces;		/* number of allocated faces */

		Halfedge free_edge;		/* pointer to the first free edge */
		Face free_face;		/* pointer to the first free face */
	}
	...
	private class Del_Point2d implements Cloneable, Comparable {
		...
		@Override
		public int compareTo(Object obj) {
			Del_Point2d that = (Del_Point2d)obj;
			if (this.x < that.x)
				return -1;
			if (this.x > that.x)
				return 1;
	...
	Delaunay(ArrayList<Point> points) {
		Point point_array[] = new Point[points.size()];
		points.toArray(point_array);
	...
	private void build_halfedge_face(DelaunaySegment del, Halfedge d) {
		...
		assert (null != new_faces && (del.num_faces == 0 || null != del.faces));
	...
	void retrieve_triangle_points(Point[] triangle_points) {
